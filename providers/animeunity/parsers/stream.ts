...
function buildFallbackMasterUrl(html: string, embedUrl: string): string | null {
  const id =
...
export function extractDownloadUrl(html: string): string | null {
...
export function extractVixCloudStreams(
  html: string,
  embedUrl: string,
  userAgent: string
): Stream[] {
  ...
  const streamHeaders = buildStreamHeaders(embedUrl, userAgent);
